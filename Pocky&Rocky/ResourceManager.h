...
public:
	...
	*/
	Sint32 loadAndGetGraphicID(SDL_Renderer* Renderer, const char* file);
...
private:
	...
	/*!
		...
		\return -1 if there's an error when loading
	*/
	Sint32 addGraphic(SDL_Renderer* Renderer, const char* file);
	...
	*/
	...
	// RAFEL: Nada de Surface. Esto lo has añadido tu. Diej que NO SE TOCA el .h de Resource Manager.
	//SDL_Surface* Image;
	std::vector<SDL_Texture*>	_TexureVector;
	//std::vector<SDL_Surface*>	_GraphicsVector;	/*!<  Vector that stores Surfaces. Useful in render methods and sequential access*/
